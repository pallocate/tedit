...
class KRelationSelector (frame : Frame, user : KUser = KUser.void()) : JDialog( frame, Lang.word( 330 ), true ), ActionListener
{
   internal var selectedUser                                    = KUser.void()
   internal var selectedRelation                                = KRelation.void()
   ...
   init
   {
      ...
      /* Users */
      if (user.relations.isEmpty())
      {
         val userList = Session.users.userList

         if(userList.isEmpty())
            throw Exception( "No users found!" )
         else
         {
            userCombo.setModel( DefaultComboBoxModel( userList.toTypedArray<KUser>() ) )
   ...
   override fun actionPerformed (e : ActionEvent)
   {
      ...
      when (actionCommand)
      {
         ...
         OK->
         {
            ...
            if (selected is KRelation && !selected.isVoid())
            {
               Session.users.activate( selectedUser.me.id )
         ...
         CANCEL ->
         {
            selectedRelation = KRelation.void()
            setVisible( false )
            dispose()
         }
         else -> {}
...
//      userCombo.setEnabled( false )
      ...
      val relations = selectedUser.relations

      if (relations.size == 0)
         throw Exception( "No relations" )
      else
      {
         relationCombo.setModel(DefaultComboBoxModel( relations.toTypedArray() ))
